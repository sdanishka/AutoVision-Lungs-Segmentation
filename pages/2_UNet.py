...
with st.sidebar:
    description = ("UNet is a convolutional neural network architecture designed for biomedical image segmentation "
                   "tasks. It consists of an encoding path that extracts features through convolutions and "
                   "downsampling, and a decoding path that restores the image resolution through upsampling and "
                   "information merging. UNet's unique architecture effectively combines local image details with "
                   "global contextual information, resulting in improved segmentation accuracy.")
    model_result = ('The UNet model in the Modern Lung Segmentation application achieved an Intersection over Union ('
                    'IoU) of {:.3f} during testing, with a training time of {:.3f}s.').format(test_iou, testing_time)
